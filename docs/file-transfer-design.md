...
- 2026-05-15: Optimization #1 — MD5 探测替代版本号比对：snippet 注入检测由 `__FT_SNIPPET_VERSION__` 字符串比对改为脚本文件 MD5 比对。本地端 `hashlib.md5(content.encode()).hexdigest()`，远端 `md5sum /tmp/ts-ft.sh | awk '{print $1}'`（兼容 macOS `md5 -q`）。优势：① 无需手动维护版本字符串；② 任何脚本修改（含注释/空格）自动触发重注入；③ 更可靠（版本号依赖 shell 变量展开，MD5 直接对文件计算）。变更文件：`snippet_registry.py`（新增 `get_script_md5()` + `get_md5_probe_command()` + `_extract_md5_from_output()`，`ensure_snippet_loaded()` 步骤2 改用 MD5 比对）
- 2026-05-15: Optimization #2 — 基于压缩后大小评估文件限制：PTY 传输瓶颈是压缩后数据通过 PTY 通道的时间。原 10MB 单一限制改为双阈值：① `_MAX_RAW_SIZE = 50MB` 原始大小绝对上限（防内存爆炸）；② `_MAX_TRANSFER_SIZE = 10MB` 压缩后传输上限。后端收到文件后先 gzip 试压缩，压缩后 ≤10MB 则放行（例如 20MB 日志 → gzip → 4MB → 允许）。前端从硬拒绝改为 confirm 软警告。Nginx `client_max_body_size` 12m→55m。变更文件：`file_transfer.py`（双阈值校验 + gzip 试算）、`pty_file_transfer.py`（原始上限 + 传输上限软警告）、`FileTransferPanel.tsx`（confirm 对话框 + 绝对上限硬拒绝）、`nginx.conf`（放宽 body 限制）
- 2026-05-15: UI Optimization — FileTransferPanel Tab 式布局重构：从左右分栏改为 Tab 切换（上传/下载/记录），核心改动：① 布局重构：三个独立 Tab 页各占全宽，280px 高度下信息密度最大化；② 视觉层级优化：标题字号 10px→11px、进度条高度 h-1.5→h-2、对比度提升（gray-700→gray-600）、输入框增加 focus ring；③ Toast 自动消失（5s fade-out + 入场动画）替代手动关闭状态栏；④ 路径记忆（localStorage 持久化上传/下载路径）；⑤ 历史增强：清除全部按钮 + 点击记录自动填充下载路径 + 带秒数时间戳 + 类型色块标识 + 空状态引导图文；⑥ 拖拽区优化：hover 微缩放 + "点击选择"独立样式 + 未连接时 disabled 态；⑦ 子组件拆分为 `_TabButton`/`_UploadTab`/`_DownloadTab`/`_HistoryTab`/`_StepIndicator`；⑧ 下载 Tab 增加空状态引导和路径示例提示
