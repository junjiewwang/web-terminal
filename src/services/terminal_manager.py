...
class TerminalManager:
    ...
    async def switch_backend(self, new_backend: TerminalBackend) -> list[str]:
        ...
        async with self._lock:
            ...
            stopped_names = [s.instance_name for s in sessions]
    ...
    async def create_session(
        ...
        async with self._lock:
            ...
            if existing is not None:
                previous_session = self._sessions.pop(storage_key, None)
        ...
        try:
            ...
            async with self._lock:
                current = self._sessions.get(storage_key)
    ...
    async def stop_session_by_id(self, session_id: str) -> bool:
        ...
        async with self._lock:
            ...
            if target_key is None:
                return False
            session = self._sessions.pop(target_key)

        await session.stop()
        logger.info("终端会话已停止(by_id): %s", session_id[:8])
        return True
